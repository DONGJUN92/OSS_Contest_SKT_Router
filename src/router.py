...
class LPBRouter:
    ...
    def fit(self, ds, train_idx: np.ndarray, tier: str):
        ...
        # ★ D70 (Phase 21) — **여기에 경고를 넣으려다 되돌렸다. 그 판단을 기록해 둔다.**
        #
        # 증상: `demo.py` 가 인코더를 익명으로 쓰고 버려(`get_encoder(...).encode(...)`)
        # `ds.text_encoder` 가 없었고, 텍스트 프롬프트를 넘기자 어댑터가 즉시 죽었다.
        # 저장소를 훑으니 **21개 파일이 전부 같았다** — D36 이 만든 계약이 소비 지점 어디에도
        # 적용돼 있지 않았다. 오프라인 경로는 특성 행렬만 쓰므로 9 Phase 동안 무증상이었다.
        #
        # 첫 처방은 "여기서 한 번 경고하면 모든 소비 지점이 동시에 덮인다"였다. **틀렸고,
        # 그것을 직감이 아니라 측정으로 확인했다** — 경고를 넣고 `pytest -W error::RuntimeWarning`
        # 을 돌리니 **108개 중 41개가 실패**했다. 전부 오탐이다: **이미 인코딩된 특성 벡터를
        # 넘기는 것은 정당한 지원 모드**이고(ablation·합성 세계·회귀 테스트가 전부 그렇다).
        # 즉 이 경고는 결함 1건을 막는 대가로 **정상 사용의 38%에 거짓 경보**를 울린다.
        # 잡음 경고는 사람을 경고에 둔감하게 만들어 **다음 진짜 경고를 죽인다.**
        # → 계약 위반은 `SubmissionRouter._features` 가 **정확히 그 순간**(텍스트 입력 + 인코더
        #   부재) 실행 가능한 메시지로 잡는다. 그것이 이미 올바른 기구다.
        # 조치는 두 가지로 갈랐다: ① 21개 소비 지점 일괄 수정 ② 데모의 배포 경로를 회귀에 고정.
        #
        # 교훈(D71): **"모든 곳을 한 번에 덮는 기구"가 언제나 옳지는 않다.** 그 기구가
        # 정당한 다수 경우를 오탐하면, 결함 하나를 막는 대가로 경고 체계 전체를 마비시킨다.
