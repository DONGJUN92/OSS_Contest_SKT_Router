...
> ⚠️ **읽기 전에 — 이 저장소가 스스로 밝힌 가장 중요한 한계**
> 검증기의 헤드라인 AUC 0.867 은 **특성 설계가 이 데이터를 본 뒤 나온 값**이다. 도메인을
> 통째로 뺀 leave-one-benchmark-out 으로 재면 평균 **0.7543**, 설계 대상 밖 벤치마크에서는
> **0.5206(무작위)** 이다. 교차점이 ≈0.885 이므로 **처음 보는 도메인에서는 순차 관측 기구가
> 순가치를 내지 못한다** — 그리고 SKT 비공개셋 도메인은 그 경우에 해당한다.
> 그래서 배포 정책을 **받은 데이터에서 직접 비교해 고르는 게이트**가 필수 안전장치다.
> 근거: `SUBMISSION.md` §3.2 · `eval/results/probe_verifier_real.json`.
...
- 회귀 방벽: `pytest tests/ -q` → **86/86** (규칙 불변식 포함 — 예산 초과·**미호출 모델 답
  ...
  ·λ 선택 래칫))
- 상세 수치·결함 수정 이력(D1~D62, R1·R2): `docs/reflections/phase1~20.md`

### 스스로 찾은 한계 (Phase 17 레드팀 — 숨기지 않는 것이 이 저장소의 규칙)

> **Phase 20 재산출 반영 완료.** 그리고 **Phase 20 결함 20여종(D34~D62)은 자체 발견이 아니라
...
| cascade-routing(결합 라우팅)이 LPB를 앞섬 | 합성 0.5091 vs **0.5024** (격차 **0.0067 로 확대**) | 후보로 승격(`SelectiveRouter`), **미역전** |
